...
AddEventHandler('loaf_housing:enterHouse', function(id)
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(result)
        MySQL.Async.fetchAll("SELECT * FROM weeds WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(weeds)
        ...
        if house.houseId == id then
            for k, v in pairs(Config.HouseSpawns) do
                if not v['taken'] then
                    ...
                    for i = 1, #weeds do   
                        newx = weeds[i].x - v['coords'].x + weeds[i].x
                        fark = weeds[i].x - v['coords'].x
                        print(fark)
                        if fark > 40 then
                        MySQL.Async.execute('UPDATE `weeds` SET `x` = @newx WHERE `x` = @oldx', {
                            ['@newx'] = newx,
                            ['@oldx'] = weeds[i].x
                        })
                    end
